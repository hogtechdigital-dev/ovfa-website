        ...
        <div style={{ maxWidth: 1152, margin: "0 auto" }}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {values.map((v, i) => {
              ...
              return (
                <div key={v.title} className="rounded-lg p-6 card-hover reveal" style={{ background: "var(--forest-muted, #265C2A)", borderTop: `4px solid ${accent}`, borderLeft: "1px solid rgba(184,146,42,0.15)", borderRight: "1px solid rgba(184,146,42,0.15)", borderBottom: "1px solid rgba(184,146,42,0.15)", transitionDelay: `${i * 80}ms` }}>
          ...
          <div className="timeline">
            {[
              ...
              <div key={s.heading} className={si % 2 === 0 ? "tl-item reveal-left" : "tl-item reveal-right"}>
                ...
                {s.body.split("\n\n").map((para, i) => (
                  <p key={i} className={`tl-body${si === 0 && i === 0 ? " drop-cap" : ""}`} style={{ marginBottom: 14 }}>{para}</p>
                ))}
              </div>
            ))}
          </div>
